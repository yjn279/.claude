...
worktree はベースブランチ（既定 `origin/main` ）の最新を起点に、リポジトリ配下の `.worktrees/` へ配置する。ディレクトリ名はブランチ名の `/` を `-` に変換した文字列とし、同名の worktree が既にあれば再利用する。現在の作業場所がすでに worktree であれば、新たな worktree もブランチも作らず、その worktree のブランチをそのまま使う。
