...
判別役（Discriminator）が本人の文章か生成文かを正しく当てた回数（正答数）と判定の総数を
受け取り、正答率（見破られ率）が「当てずっぽう（50%）と有意差なし」と言えるかどうかを
両側二項検定で調べる。見破られ率が閾値未満かどうかの収束判定は epoch.py が担い、
このスクリプトは正答率と p 値の計算だけを行う。しきい値による合否の判断はここでは行わず、
呼び出し側に委ねる。
