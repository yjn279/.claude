...
呼び出し）が正しく当てた回数（正答数）と判定の総数を受け取り、正答率（見破られ率）が
「当てずっぽう（50%）と有意差なし」と言えるかどうかを両側二項検定で調べる。見破られ率が
閾値未満かどうかの収束判定は epoch.py が担い、このスクリプトは正答率と p 値の計算だけを
行う。しきい値による合否の判断はここでは行わず、呼び出し側に委ねる。

入力（コマンドライン引数）:
    --total   （必須）判定の総数
    --correct （必須）Discriminator が正しく本人の文章を当てた回数
