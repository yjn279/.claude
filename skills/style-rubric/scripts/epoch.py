...
判別役（Discriminator）は、本人の文章と生成文を半々に混ぜたバッチを1サンプルずつ判定する。
その判定一覧（各サンプルの正解が本人か生成文か、判別役が本人と答えたか生成文と答えたか）を受け取り、
混同行列を数え上げたうえで、見破られ率（判別役の正答率）と、その正答率が偶然（50%）と
...
判別役が中身を見ずに全部「生成文」と答えるだけでも見破られ率はちょうど 0.5 になってしまう、
...
入力（標準入力または引数のファイルパスから読む JSON）:
    ...
    - "verdict" （必須）判別役の回答。"本人" または "生成文"

出力（標準出力への JSON、終了コード0）:
    - "total"               判定の総数
    - "confusion"           混同行列。正解（"genuine" = 本人 / "fake" = 生成文）を外側、
                             判別役の回答を内側のキーとする、4つの数の入れ子
    - "deception_rate"      見破られ率（判別役の正答率）
    - "p_value"             見破られ率が50%と有意差なしという仮説のもとでの両側 p 値
    - "genuine_answer_rate" 判別役が「本人」と答えた割合
...
def check_balance(confusion, total):
    ...
    if not (low <= genuine_answer_rate <= high):
        raise EpochError(
            "判別役の回答が一方へ倒れています。「本人」と答えた割合は"
            f"{genuine_answer_rate:.3f}で、許容範囲{low}〜{high}の外です。"
            "見破られ率は解釈できないため、判定を見直すか判別役のプロンプトを確認してください。"
        )
