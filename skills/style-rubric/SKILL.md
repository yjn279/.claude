...
| Discriminator（判別役） | 見本を参照し、提示された1件の文章が本人の筆か生成文かを判定する言語モデル呼び出し |
...
| エポック | 1ラウンド。1バッチ分の生成と判別、および1回のパラメータ更新からなる |
...
閾値の既定値は見破られ率 0.6、バッチサイズの既定値は 20 とする。1エポックは次の手順からなり、エポック数に上限は設けない。収束するか、後述のアサーションが崩れて停止するまで回り続ける。収束しないまま回り続ける場合は `report.md` の推移を見て人が止める。
...
5. **収束判定**：異常終了しなければ、`epoch_result.json` の `deception_rate` を見る。
   ...
   - **0.6以上**：バッチ内の全判定根拠を `prompts/rubric.md` の `<verdict_reasons>` へ渡し、直前のルーブリックを `<previous_rubric>` へ、学習データを `<training_data>` へ渡してルーブリックを更新する。更新は `prompts/rubric.md` が定める手順（3件以上のサンプルに共通する特徴だけを採用する）に従う。更新後のルーブリックを持って次のエポックへ進む。
...
| 見破られ率が上下して収束しない | バッチサイズ不足 | バッチサイズ（既定20）を40以上に増やす |
...
Discriminator のアンサンブル化は初版では実装しない。単一の `prompts/discriminator.md` で回し、上表の局所解の症状が観測されたときに導入する。導入するときは、着眼点（語彙と語形、統語とリズム、話題選好など）ごとに `prompts/discriminator.md` のバリエーションを用意し、同じサンプルを全バリエーションへ判定させ、多数決またはラウンドロビンでの交代に切り替える。集計は各バリエーションの判定を `{"truth": ..., "verdict": ...}` の一覧にまとめたうえで、そのまま `scripts/epoch.py` へ渡せばよい。
