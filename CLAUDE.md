...
助言役ツール `advisor` は、会話全体を読んだうえで助言を返す相談相手である。実質的な作業に入る前、行き詰まったとき、方針を変えるとき、完了と判断したときに自分から呼ぶ。完了時は、成果物を保存・コミットして形に残してから呼ぶ。

返ってきた助言は重く受け止める。実際に試して失敗したか、一次情報が助言の特定の主張を否定するときに限り、根拠を示して外れる。自分が調べた事実が助言と異なる方向を指すだけで、特定の主張を否定するとまでは言えないときは、黙ってどちらかに寄せず、食い違いを示して改めて助言を求める。
